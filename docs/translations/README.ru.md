...
Полнофункциональная встраиваемая система тикетов поддержки для Laravel. Добавьте её в любое приложение — получите полноценный хелпдеск с отслеживанием SLA, правилами эскалации, рабочими процессами агентов и клиентским порталом. Внешние сервисы не требуются.
...
- **Ticket snooze** — Откладывание тикетов с предустановками (1ч, 4ч, завтра, на следующей неделе); команда Artisan `escalated:wake-snoozed-tickets` автоматически пробуждает их по расписанию
...
- **Embeddable support widget** — Лёгкий виджет `<script>`, обслуживаемый через маршруты `/support/widget/*` с поиском по базе знаний, формой тикета и проверкой статуса
...
- **Real-time broadcasting** — Опциональная трансляция через Pusher, Reverb или Soketi с автоматическим откатом на опрос
- **Переключатель базы знаний** — Включение или отключение публичной базы знаний из настроек администратора
- **CI: Laravel Pint** — Автоматическое применение стиля кода при каждом пулл-реквесте
...
Все данные тикетов проксируются через облачный API. Ваше приложение обрабатывает аутентификацию и рендерит UI, но хранилище находится в облаке. Поддерживает несколько доменов на один API-ключ.
...
- **[Escalated for Laravel](https://github.com/escalated-dev/escalated-laravel)** — Пакет Laravel Composer (вы здесь)
- **[Escalated for Rails](https://github.com/escalated-dev/escalated-rails)** — Движок Ruby on Rails
